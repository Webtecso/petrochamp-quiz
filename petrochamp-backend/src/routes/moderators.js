...
function sanitizeAreas(input) {
    if (!Array.isArray(input))
        return [];
    return Array.from(new Set(input.filter((a) => VALID_AREAS.includes(a))));
}
router.get('/', async (_req, res) => {
    const moderators = await db_1.prisma.moderator.findMany({
...
router.post('/', async (req, res) => {
    const { name, role, areas } = req.body;
    if (!name) {
        res.status(400).json({ error: 'name é obrigatório' });
        return;
    }
    if (role === 'principal') {
        const existingPrincipal = await db_1.prisma.moderator.findFirst({
    ...
    let code = (0, liveState_1.generateModeratorCode)();
    for (let i = 0; i < 5; i++) {
        const exists = await db_1.prisma.moderator.findUnique({ where: { code } });
        if (!exists)
            break;
...
router.put('/:id', async (req, res) => {
    ...
    try {
        ...
        const moderator = await db_1.prisma.moderator.update({
            where: { id },
            data: {
                name,
                role: finalRole,
...
router.delete('/:id', async (req, res) => {
    const { id } = req.params;
    try {
        await db_1.prisma.moderator.update({ where: { id }, data: { deletedAt: new Date() } });
        res.status(204).send();
    }
    catch {
        res.status(404).json({ error: 'Moderador não encontrado' });
